...
эвристики, то<br>
find = O(log(n)), merge = O(find) = O(log(n))
#### Сжатия путей
Когда мы ищем родителя элемента, мы можем переподвесить элементы напрямую к корню,
таким образом уменьшая средний случай поиска родителя и ускоряя find.
Доказывается, что не используя другие эвристики:<br>
$$O(find) = O(\frac{log(\frac{n^2}{m})}{log(\frac{n}{m})}) = O(log(n))$$
