...
> [!NOTE] Почему на алгоритме Дейкстры нельзя строить граф с отрицательными весами?
...
> [!WARNING] Второй ворнинг. Оказалось, есть 2 реализации этого алгоритма. Версия, которую
...
> [!WARNING] В душе не чаю, как работает этот алгоритм и откуда берется асимптотика.
...
### Построение строки
> [!NOTE] Могу спокойно дать строку, которая сломает алгос.
...
> [!NOTE] Мне кажется, что можно обойтись и без #...
### Асимптотика
