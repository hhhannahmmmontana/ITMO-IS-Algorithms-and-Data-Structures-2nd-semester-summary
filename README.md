...
ребро как минимальное для вершины j. Затем ищем вершину с минимальным ключом и повторяем
этот проход по ребрам |V| раз (считая стартовый проход). На выходе получаем массив prev,
из которого можно получить MST.
### Асимптотика
